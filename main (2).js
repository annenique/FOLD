...
function loadLevel(key) {
  ...
  // Reset sliders (use correct IDs matching index.html)
  ...
  const mutInfo = document.getElementById('mutInfo');
  if (mutBtn) { mutBtn.textContent = 'APPLY MUTATION'; mutBtn.dataset.active = 'false'; mutBtn.classList.remove('active'); }
  if (mutInfo) mutInfo.style.display = 'none';

  // Update protein context panel
  if (window.updatePCtx) window.updatePCtx(key);
  ...
  if (p) setTimeout(()=>window.showSci&&window.showSci(p.icon||'🔬',p.title,p.body,p.src), 800);
}
...
function updateIntegrityUI(score, label, color) {
  ...
  const stat = document.getElementById('intStatus');
  if (!fill || !stat) return;
...
function applyParamsToProtein(temp, pH, salt) {
  ...
  currentProtein.scale.setScalar(1 + denatureFrac*0.35);
  ...
  if (temp > 65 && !currentProtein.userData.heatPopupShown) {
    ...
    if (p) window.showSci&&window.showSci(p.icon||'🌡️', p.title, p.body, p.src);
  }
  if (temp <= 50) currentProtein.userData.heatPopupShown = false;
}

// Expose param and mutation functions to HTML layer
window._applyP = function(temp, pH, salt) {
  applyParamsToProtein(temp, pH, salt);
};

window._applyM = function(btn) {
  if (!currentProtein) return;
  mutationActive = !mutationActive;
  btn.classList.toggle('active', mutationActive);
  const mut  = currentProtein.userData.mutation;
  const info = document.getElementById('mutInfo');
  if (mutationActive) {
    btn.textContent = 'REVERT MUTATION';
    if (info) { info.style.display = 'block'; info.textContent = 'Applied: '+mut.name; }
    const p = currentProtein.userData.popups.find(p=>p.trigger==='mutation');
    if (p) setTimeout(()=>window.showSci&&window.showSci(p.icon||'⚡',p.title,p.body,p.src), 300);
  } else {
    btn.textContent = 'APPLY MUTATION';
    if (info) info.style.display = 'none';
  }
  applyParamsToProtein(
