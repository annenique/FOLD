...
document.querySelectorAll('.protein-card').forEach(card=>{
  card.addEventListener('click',()=> loadLevel(card.dataset.level));
});
...
document.getElementById('tempSlider').addEventListener('input', function() {
  ...
  let effect='';
  if(t<10)       effect='Near freezing. Kinetics slow. Some proteins precipitate.';
  else if(t<25)  effect='Cold. Protein stable but slow-moving.';
  else if(t<42)  effect='Physiological range. Normal folded state.';
  else if(t<55)  effect='Fever range. Some proteins begin to stress.';
  else if(t<70)  effect='⚠️ Denaturing! Hydrogen bonds breaking. Helices unraveling.';
  else if(t<85)  effect='⛔ Severely denatured. Most secondary structure lost.';
  else           effect='☠️ Complete denaturation. Random coil. Function lost.';
  document.getElementById('tempEffect').textContent = effect;
  const pH   = parseFloat(document.getElementById('pHSlider').value);
  const salt = parseInt(document.getElementById('saltSlider').value);
  applyParamsToProtein(t, pH, salt);
});
...
document.getElementById('pHSlider').addEventListener('input', function() {
  ...
  let effect='';
  if(pH<2)       effect='Strongly acidic. Protonating carboxylates. Severe denaturation.';
  else if(pH<5)  effect='Acidic. Charged AA side chains altered. Structure stressed.';
  else if(pH<6.5)effect='Slightly acidic. His residues becoming protonated.';
  else if(pH<8)  effect='Physiological. Charges balanced. Normal function.';
  else if(pH<10) effect='Basic. Lys/Arg losing positive charge. Structure stressed.';
  else           effect='Strongly basic. Severe disruption of ionic interactions.';
  document.getElementById('pHEffect').textContent = effect;
...
// Salt slider
document.getElementById('saltSlider').addEventListener('input', function() {
  const s = parseInt(this.value);
  document.getElementById('saltVal').textContent = s+'mM';
  let effect='';
  if(s<30)       effect='Hypotonic. Reduced ionic shielding. Charge repulsion increases.';
  else if(s<100) effect='Low salt. Some charge interactions weakened.';
  else if(s<250) effect='Physiological range. Normal ionic environment.';
  else if(s<400) effect='High salt. Hydrophobic effect strengthened (salting-out begins).';
  else           effect='⚠️ Very high salt. Kosmotropic ions displacing water. Aggregation risk.';
  document.getElementById('saltEffect').textContent = effect;
  const temp = parseInt(document.getElementById('tempSlider').value);
  const pH   = parseFloat(document.getElementById('pHSlider').value);
  applyParamsToProtein(temp, pH, s);
});
...
document.getElementById('mutationBtn').addEventListener('click', function() {
  ...
  this.dataset.active = mutationActive;
  this.classList.toggle('active', mutationActive);

  const mut  = currentProtein.userData.mutation;
  const info = document.getElementById('mutationInfo');

  if (mutationActive) {
    this.textContent = '↩ REVERT MUTATION';
    info.style.display  = 'block';
    info.textContent    = '⚡ Mutation applied: '+mut.name+' at '+mut.residue;
    // Visually mark mutation site
    currentProtein.traverse(child => {
      if (child.isMesh && child.userData.mutationSite) {
        child.material.color.setHex(mut.color);
    ...
    this.textContent = '⚡ APPLY: '+mut.name;
